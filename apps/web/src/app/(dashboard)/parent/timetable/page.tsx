...
export default function ParentTimetablePage() {
  ...
  return (
    <DashboardLayout user={user} pageTitle="Child's Timetable">
      ...
      <Card className="overflow-x-auto">
        <CardHeader>
          <CardTitle>Weekly Schedule — Class 3B</CardTitle>
          <span className="text-xs text-muted">Academic Year 2024–25</span>
        </CardHeader>
        <table className="w-full text-sm min-w-[700px]">
          <thead>
            <tr className="border-b border-border">
              <th className="text-left py-3 pr-4 text-muted font-medium w-20">Time</th>
              {DAYS.map((d) => <th key={d} className="text-center py-3 px-2 text-muted font-medium">{d.slice(0, 3)}</th>)}
            </tr>
          </thead>
          <tbody>
            {HOURS.map((hour) => {
