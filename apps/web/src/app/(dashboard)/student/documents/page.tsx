...
export default function StudentDocumentsPage() {
  ...
  async function handleRequest() {
    ...
    try {
      ...
      setIsOpen(false);
      setForm((p) => ({ ...p, type: DOC_TYPES[0], note: '' }));
      refetch();
    ...
    <DashboardLayout user={user} pageTitle="Document Requests">
      {student && (
        <div className="mb-4 px-4 py-3 bg-secondary/5 border border-secondary/20 rounded-card text-sm text-secondary font-medium">
          Requesting for: <span className="font-bold">{student.user.firstName} {student.user.lastName}</span>
          {student.classEnrollments?.[0] && <> · Class {student.classEnrollments[0].class.name}</>}
        </div>
      )}
